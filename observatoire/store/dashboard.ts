...
export const mutations: MutationTree<DashboardState> = {
  ...
  DENSITE_AIRES: (state, densiteAires: DashboardState["densiteAires"]) => (state.densiteAires = densiteAires),
...
export const actions: ActionTree<DashboardState, DashboardState> = {
  ...
  async getDensitePeriod({ state, commit }){
    
    const period = { 
      start: new Date(Number(state.period.year),Number(state.period.month) - 1,1),
      end: new Date(Number(state.period.year),Number(state.period.month),0)
    }
