...
export default class occupationHandler {
  ...
  static async occupationMonthly(request: FastifyRequest<occupationTypes.monthly>, reply: FastifyReply):Promise<void>{
    try {
      ...
      const sql = `SELECT a.territory, b.l_territory, a.journeys, a.passengers, a.occupation_rate, ST_AsGeoJSON(b.geom,6)::json as geom 
      FROM covoiturage.journeys_monthly_occupation_rate a
      LEFT JOIN perimeters.territories_points b on a.territory = b.territory AND a.type = b.type
